...
class TermTupleEnumeratorBase : public TermTupleEnumeratorInterface
{
 public:
  ...
  virtual void initLogger(size_t variableIx, size_t termsSize);
  virtual bool isInRelevantDomain(Node term);
 ...
 protected:
  ...
  bool d_anyTerms; // keep track of whether any terms where added
};
...
//  std::map<size_t, std::vector<Node> > d_termDbList_var;
...
/**
 * Enumerate ground terms as they come from the term database
 * Use ML to reorder the terms, in a hopefully better way.
 */

class TermTupleEnumeratorBasicML : public TermTupleEnumeratorBase
{
 public:
  TermTupleEnumeratorBasicML(Node quantifier,
                           const TermTupleEnumeratorEnv* env,
                           QuantifiersState& qs, LearningGlobalContext* lctxt)
      : TermTupleEnumeratorBase(quantifier, env),
        d_qs(qs),
        d_tdb(env->d_tr->getTermDatabase())
  {
    d_learncontext = lctxt;
  }

  virtual ~TermTupleEnumeratorBasicML() = default;

 protected:
  /**  a list of terms for each type */
  std::map<TypeNode, std::vector<Node> > d_termDbList;
  /** a list of terms for each variable so we can instantiate
   * variables of the same type differently */
  std::map<size_t, std::vector<Node> > d_termDbList_variables;
  /** keep track of which types we already passed */
  std::map<TypeNode, bool> d_types_already_prepared;
  // probably should define this outside of class when cleaning up code

  // don't think using both virtual and override in one place makes sense but following style
  // of other code to be sure.
  virtual size_t prepareTerms(size_t variableIx) override
  {
    int missing_terms = 0;
    int total_terms = 0;
    const TypeNode type_node = d_typeCache[variableIx];
    // TODO This will not work as desired; if two variables have the same
    // type, the termdblist for the second variable of a type will not be filled.
...
//          {
//            repsFound[rep] = gt;
//            std::cout << gt.getId() << "ADDED TO LISTS" << std::endl;
//            d_termDbList[type_node].push_back(gt);
//            d_termDbList_variables[variableIx].push_back(gt);
//          }
//        }
//      }
//    };
...
//
//    std::cout << d_termDbList_variables.size() << std::endl;
//    std::cout << ContainsKey(d_termDbList_variables, variableIx) << std::endl;
//    for (auto& [k, v] : d_termDbList_variables) {
//      std::cout << k << " -> [ ";
//
//      for (auto& t : v) {
//        std::cout << t << ", " ;
...
//    }
//    std::cout << ContainsKey(d_termDbList_variables, variableIx) << std::endl;
    if (!ContainsKey(d_termDbList_variables, variableIx))
    {
//      std::cout << "Termdblistvariables did not contain variable " << variableIx << std::endl;
      const size_t ground_terms_count = d_tdb->getNumTypeGroundTerms(type_node);
      std::map<Node, Node> repsFound;
      for (size_t j = 0; j < ground_terms_count; j++)
      {
        Node gt = d_tdb->getTypeGroundTerm(type_node, j);
//        std::cout << "Term " << j << " " << gt << std::endl;
        if (!quantifiers::TermUtil::hasInstConstAttr(gt))
        {
//          std::cout << "Term did not have instconstatt" << std::endl;
          Node rep = d_qs.getRepresentative(gt);
          if (repsFound.find(rep) == repsFound.end())
          {
//            std::cout << "We don't have a term of this eq class, adding it" << std::endl;
            repsFound[rep] = gt;

//             Prevent duplicate terms in type list -> this is probably wrong, because we don't add new eq classes of the same type?
            if (!ContainsKey(d_types_already_prepared, type_node)) {

//              std::cout << "Term added to termlist" << std::endl;
              d_termDbList[type_node].push_back(gt);
            }
//            std::cout << "Term added to variabletermlist" << std::endl;
            d_termDbList_variables[variableIx].push_back(gt);
          }
//          else {
//            std::cout << "We already have a term of this equivalence class" << std::endl;
//          }
        }
      }
    };
    // When a variable of the same type has already been prepared, we don't want to again
    // add stuff to d_termDbList
    d_types_already_prepared[type_node] = true;
//    std::cout << "2Checking list sizes"  << std::endl;
//    std::cout << d_termDbList.at(type_node).size() << " " << d_termDbList_variables.at(variableIx).size() << std::endl;
    if (! (d_termDbList[type_node].size() == 0 or d_termDbList_variables[variableIx].size() == 0  ))
    {
//      std::cout << "both lists have elements" << std::endl;
      int var_node_id = d_quantifier[0][variableIx].getId();

      int normalized_quant_node_id =
          (*d_learncontext->d_node_cvc_graph_index).at(var_node_id);

//      std::cout << "quant id" << std::endl;
      std::vector<int> var_ids = {normalized_quant_node_id};
      std::vector<int> term_ids;
      std::vector<int> orig_term_ids;

//      std::cout << "TERMSSSSS : " << d_termDbList_variables[variableIx].size()
//                << std::endl;
//      std::cout << "TERMSSSSS : " << d_termDbList[type_node].size()
//                << std::endl;
      //    auto (*d_learncontext->d_node_cvc_graph_index)[t.getId()]
      std::vector<double> preds ;
//      td::chrono::time_point<std::chrono::high_resolution_clock> start_predlookup = std::chrono::high_resolution_clock::now();


      // I can put it on rails here.
      for (auto& t : d_termDbList_variables[variableIx])
      {

        int term_id = t.getId(); //
//        std::cout << t << std::endl;
//        std::cout << term_id << std::endl;
//        std::cout << t.getKind() << std::endl;
//        std::cout << static_cast<int>(t.getKind()) << std::endl;
//        std::cout << "Accessing map now" << std::endl; //
//        std::cout << (*d_learncontext->d_node_cvc_graph_index).at(term_id)
//                  << std::endl;

        std::pair<int,int> keypair = std::make_pair(var_node_id, term_id);
//        std::cout << "Doing score lookup" << "(" << var_node_id << ", " << term_id << ")" <<  std::endl;

        preds.push_back(d_learncontext->d_varterm_score_lookup->at(keypair));

//        auto node_remapper = (*d_learncontext->d_node_cvc_graph_index);
//        if ( ContainsKey(node_remapper, t.getId())) // skolem = 9
//        {
//          term_ids.push_back(node_remapper.at(t.getId()));
//          //      orig_term_ids.push_back(t.getId());
//
//        }
//        else {
//          // if we did not find the term, put it into a list and check if they
//          // are in the graph next quantifier round
//          std::cout << "Else branch" << std::endl;
//          missing_terms += 1;
//          if (ContainsKey(d_learncontext->d_orphan_terms, var_node_id)) {
//            d_learncontext->d_orphan_terms.at(var_node_id).insert(static_cast<int>(t.getId()));
//          }
//          else {
//            std::set<int> c_set({static_cast<int>(t.getId())});
//            d_learncontext->d_orphan_terms[var_node_id] = c_set;
//          }
//
//        }
        total_terms += 1;
      ...
      //TODO Perhaps we can try to return somehow if there is no suitable term according to the network (premsel)

//      std::cout << "Missing terms: " << var_node_id << " " << missing_terms << " of " << total_terms << std::endl;
      //    std::cout << "Term IDS: " << term_ids << std::endl;
//      std::vector<double> predictions =
//          d_learncontext->d_graphtermPredictor->rank_terms(var_ids, term_ids);
      //    std::vector<int> term

      // Now reorder the terms according to the GNN predictions;

      std::vector<std::pair<double, Node>> combined_structure;
      Assert(preds.size() == d_termDbList_variables[variableIx].size());
//      std::cout << "PBEFORESORT "  << std::endl;

      // TODO implement sampling here

      auto num_preds = preds.size();
      for (size_t i = 0; i < num_preds; ++i)
      {
//
//        std::cout << "P " << preds[i] << " " << d_termDbList_variables.at(variableIx)[i] << std::endl;
        combined_structure.push_back(std::make_pair(
            preds[i], d_termDbList_variables.at(variableIx)[i]));
      }

      std::sort(combined_structure.begin(),
                combined_structure.end(),
                [](const auto& i, const auto& j) { return i.first > j.first; });

      std::vector<Node> new_order;
//      std::cout << "PAFTERSORT "  << std::endl;
      for (auto& [pred, term] : combined_structure)
      {
//        std::cout << "P " << pred << " " << term << std::endl;
        new_order.push_back(term);
      }
      // change this to measure the slowdown from all this code //
      d_termDbList_variables[variableIx] = new_order;
//      d_termDbList_variables[variableIx] = d_termDbList[d_typeCache[variableIx]];
    }
//    std::cout << "reached end of prepareterms" << std::endl;
    return d_termDbList_variables[variableIx].size();
  }

  /** version that uses the variable lists */
  virtual Node getTerm(size_t variableIx, size_t term_index) override  {
//          std::cout << term_index << "must be smaller than" << d_termDbList_variables[variableIx].size() << std::endl;
          Assert(term_index < d_termDbList_variables[variableIx].size());
          return d_termDbList_variables[variableIx][term_index];
  };

//  /** version that uses the type lists */
//  virtual Node getTerm(size_t variableIx, size_t term_index) override  {
////    std::cout << term_index << "must be smaller than" << d_termDbList_variables[variableIx].size() << std::endl;
//    Assert(term_index < d_termDbList[d_typeCache[variableIx]].size());
//    return d_termDbList[d_typeCache[variableIx]][term_index];
//  };
  ...
  /** Pointer to term database */
  TermDb* d_tdb;
  LearningGlobalContext* d_learncontext;
};

/**
 * Enumerat
 * e ground terms according to the relevant domain class.
 */
class TermTupleEnumeratorRD : public TermTupleEnumeratorBase
{
 public:
  ...
  virtual void initLogger(size_t variableIx, size_t termsSize) override;
  virtual bool isInRelevantDomain(Node term) override;
...
void TermTupleEnumeratorBase::init()
{
  ...
  d_anyTerms = false;  // keep track of whether any terms where added 
  ...
  const bool logging = options.quantifiers.qlogging; //REM: || d_global->d_ml != nullptr; 
  ...
  for (size_t variableIx = 0; variableIx < d_variableCount; variableIx++)
  {
    ...
    // TODO Use a decoupled randomized version via prepareTermsVar
    // make an option for it.
    const size_t termsSize = prepareTerms(variableIx);
    if (logging)
    {
       initLogger(variableIx, termsSize);
    }
  ...
  if (logging && d_anyTerms)
  {
    QuantifierLogger::s_logger->increasePhase(d_quantifier);
  }
}

void TermTupleEnumeratorBase::initLogger(size_t variableIx, size_t termsSize) 
{
  for (size_t termIx = 0; termIx < termsSize; termIx++)
  {
    const auto term = getTerm(variableIx, termIx); // YAN: probably needs getTermOriginal due to the age feature
    const bool isRelevant = isInRelevantDomain(term);
    d_anyTerms = QuantifierLogger::s_logger->registerCandidate(
  ...
  if (termsSize == 0 && d_env->d_fullEffort)
  {
    ...
    const auto term = d_env->d_tr->getTermForType(typeNode);
    const bool isRelevant = isInRelevantDomain(term);
    d_anyTerms = QuantifierLogger::s_logger->registerCandidate(
                   d_quantifier, variableIx, term, isRelevant)
               || d_anyTerms;
...
bool TermTupleEnumeratorBase::isInRelevantDomain(Node term) 
{
   return false;
}

void TermTupleEnumeratorRD::initLogger(size_t variableIx, size_t termsSize) 
{
  const auto& relevantTermVector =
    d_rd->getRDomain(d_quantifier, variableIx)->d_terms;
  d_relevant_cur = std::set<Node>(relevantTermVector.begin(),
                                  relevantTermVector.end());
  TermTupleEnumeratorBase::initLogger(variableIx, termsSize);
}

bool TermTupleEnumeratorRD::isInRelevantDomain(Node term) 
{
   return ContainsKey(d_relevant_cur, term);
}
...
size_t TermTupleEnumeratorBasic::prepareTerms(size_t variableIx)
{
  ...
  // TODO we can randomize more aggressively if we decouple the variables and the types
  if (randomize) {

    std::shuffle(std::begin(d_termDbList[type_node]), std::end(d_termDbList[type_node]),  QuantifierLogger::s_logger->d_term_rng);
//    std::shuffle(std::begin(d_termDbList[type_node]), std::begin(d_termDbList[type_node]) + 5, QuantifierLogger::s_logger->d_term_rng);
  }
  // Perhaps randomize only the beginning.


//  int test_map = d_learningContext.d_node_cvc_graph_index[0];
...
//size_t TermTupleEnumeratorBasic::prepareTermsVar(size_t variableIx)
//{
//  // decouple the type of a variable from its term ranking:
//  // variables with the same type can have different rankings of trems
//  const TypeNode type_node = d_typeCache[variableIx];
//  if (!ContainsKey(d_termDbList_var, variableIx))
//  {
//    const size_t ground_terms_count = d_tdb->getNumTypeGroundTerms(type_node);
//    std::map<Node, Node> repsFound;
//    for (size_t j = 0; j < ground_terms_count; j++)
...
//        {
//          repsFound[rep] = gt;
//          d_termDbList_var[variableIx].push_back(gt);
//        }
//      }
//    }
//  }
...
//
//
//  // TODO we can randomize more aggresively if we decouple the variables and the types
//  if (randomize) {
//    QuantifierLogger::s_logger->d_term_rng.seed(std::time(0));
//    std::shuffle(std::begin(d_termDbList_var[variableIx]), std::end(d_termDbList_var[variableIx]),  QuantifierLogger::s_logger->d_term_rng);
//  }
//  //  int test_map = d_learningContext.d_node_cvc_graph_index[0];
//  Trace("inst-alg-rd") << "Instantiation Terms for child " << variableIx << ": "
//                       << d_termDbList_var[variableIx] << std::endl;
//  return d_termDbList_var[variableIx].size();
//}
...
//
//Node TermTupleEnumeratorBasic::getTermPerVariable(size_t variableIx, size_t term_index) {
////  const TypeNode type_node = d_typeCache[variableIx];
//  Assert(term_index < d_termDbList_var[variableIx].size());
//  return d_termDbList_var[variableIx][term_index];
//}
...
TermTupleEnumeratorInterface* mkTermTupleEnumeratorML(
    Node q, const TermTupleEnumeratorEnv* env, QuantifiersState& qs, LearningGlobalContext* lgcontext)
{
  return static_cast<TermTupleEnumeratorInterface*>(
      new TermTupleEnumeratorBasicML(q, env, qs, lgcontext));
}
