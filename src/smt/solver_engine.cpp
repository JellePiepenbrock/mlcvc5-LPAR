...
void SolverEngine::finishInit()
{
  ...
  quantifiers::QuantifierLogger::s_logger =
      std::make_unique<quantifiers::QuantifierLogger>(*d_env.get());
...
{
  quantifiers::QuantifierLogger::s_logger.reset(nullptr);
  ...
  for (std::pair<const Node, InstantiationList>& i : rinsts)
  {
    ...
    else
    {
      ...
      Assert(d_env->getOptions().quantifiers.printInstMode
             == options::PrintInstMode::LIST);
      if (d_env->getOptions().quantifiers.qlogging)
      {
